...
func (a *configMapsClient) listAllAccounts(ctx context.Context) (map[string]*accounts.Account, error) {
	...
	secret, err := a.k.GetSecret(ctx, types.NamespacedName{Namespace: common.GetSystemNamespace(), Name: common.EverestAccountsSecretName})
	if err != nil {
		return nil, err
	}
...
func (a *configMapsClient) salt(ctx context.Context) ([]byte, error) {
	ns, err := a.k.GetNamespace(ctx, types.NamespacedName{Name: common.GetSystemNamespace()})
	if err != nil {
		return nil, err
	}
